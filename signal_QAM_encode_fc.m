...
for m = 1:M/2
    for n = 1:M/2
        modulation_data(i) = Amc(m)+1j*Ams(n);
        i = i+1;
    end
end
...
for i = 1:N_symbol
    if signal_raw(2*i-1)==0 && signal_raw(2*i)==0
        qamSignal_sin(Tn*(i-1)+1:Tn*i) = real(signal_gt(Tn*(i-1)+1:Tn*i).*modulation_data(1));
    elseif signal_raw(2*i-1)==0 && signal_raw(2*i)==1
        qamSignal_sin(Tn*(i-1)+1:Tn*i) = real(signal_gt(Tn*(i-1)+1:Tn*i).*modulation_data(2));
    elseif signal_raw(2*i-1)==1 && signal_raw(2*i)==0
        qamSignal_sin(Tn*(i-1)+1:Tn*i) = real(signal_gt(Tn*(i-1)+1:Tn*i).*modulation_data(3));
    else
        qamSignal_sin(Tn*(i-1)+1:Tn*i) = real(signal_gt(Tn*(i-1)+1:Tn*i).*modulation_data(4));
    end
end


% 通过AWGN信道传输
pamSignalNoisy = awgn(qamSignal_sin, SNR, 'measured');

%% 时域波形绘制 - 噪声前后对比
figure;
plot(t, qamSignal_sin);
